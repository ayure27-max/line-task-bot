...
def webhook():
    ...
    for event in events:
        ...
        if clean_message == "全体追加モード":
            ...
            save_tasks(tasks)
            send_reply(reply_token, "『日付 内容』で送ってね", None)
            continue

        if clean_message == "完了モード":
            tasks["states"][user_id] = "complete"
            save_tasks(tasks)
            send_reply(reply_token, "完了する番号を送ってね（例: 1 G2 3）", None)
            continue

        if clean_message == "削除モード":
            tasks["states"][user_id] = "delete"
            save_tasks(tasks)
        ...
        if state == "complete":
            ...
            for n in nums:
                if n.startswith("G") and n[1:].isdigit():
                    ...
                    idx = int(n) - 1
                    personal = [t for t in tasks["users"][user_id] if t["status"] != "done"]
                    if 0 <= idx < len(personal):
                        personal[idx]["status"] = "done"
        ...
        if state == "delete":
            ...
            for n in nums:
                if n.startswith("G") and n[1:].isdigit() and user_id in ADMIN_USERS:
                    ...
                    personal = [t for t in tasks["users"][user_id] if t["status"] != "done"]
                    if 0 <= idx < len(personal):
                        tasks["users"][user_id].remove(personal[idx])
